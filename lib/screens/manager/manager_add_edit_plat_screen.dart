...
class _ManagerAddEditPlatScreenState extends State<ManagerAddEditPlatScreen> {
  ...
  String _categorie = 'Boisson chaude';
  ...
  void initState() {
    ...
    _prixCtrl = TextEditingController(
        text: widget.plat != null
            ? widget.plat!.prix.toStringAsFixed(0)
            : '');
    _categorie = widget.plat?.categorie ?? 'Boisson chaude';
  ...
  Future<void> _save() async {
    if (_nomCtrl.text.trim().isEmpty || _prixCtrl.text.trim().isEmpty) {
    ...
    try {
      ...
      if (finalImage.isEmpty) {
        finalImage = Plat.defaultImageFor(_categorie);
      }

      final plat = Plat(
        id: widget.plat?.id ?? const Uuid().v4(),
      ...
      if (_isEdit) {
        await platService.update(plat);
      } else {
        await platService.add(plat);
      }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border:
                        Border.all(color: kBrown.withOpacity(0.3), width: 1.5),
                  ),
            ...
            const Center(
              child: Text('Appuyer pour changer l\'image',
                  style: TextStyle(
                      fontFamily: 'LeagueSpartan',
                      fontSize: 12,
                      color: kGrey)),
    ...
    if (_imageUrl.startsWith('http')) {
      return ClipRRect(
        ...
        child: Image.network(_imageUrl,
            fit: BoxFit.cover, key: ValueKey(_imageUrl)),
      );
    }
